module.exports = {
    forum_format_date: (date) => {
      ...
      return `${monthNames[new Date(date).getMonth()]} ${new Date(date).getDate()}, ${new Date(date).getFullYear()} - ${twelveHourFormatTime}:${minutes} ${amPm}`;
    },
    calc_replies: (posts) => {
      if ((posts - 1) <= 0) {
        return 0;
    ...
    render_latest_comment: (comments) => {
      ...
      if (latestCommentTimestamp === undefined || latestCommentTimestamp === null) {
        return "No posts";
      }
      const dayDifference = Math.floor((now.getTime() - latestCommentTimestamp.getTime()) / (1000 * 60 * 60 * 24));
      if (dayDifference === 0) {
        return "Today";
